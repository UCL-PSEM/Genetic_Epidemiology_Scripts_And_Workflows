...
# adjust column numbers here to match your file structure! (this modified slightly from CLS code as nothing would map unless I set the end point to be +1 actual position. Read online something about 0-based coordinates but check)
...
#=========Modify Summary Stats to have new coordinates==========#

## Remove the chr prefix from column one and print a new simpler file that is just CHR POS RSID
awk '{gsub("chr","",$1); print $1,$2,$4}' t1d_lifted.bed > t1d_lifted_simple.txt

## Now use this new file to match the new coordinates to the original summary stats data

awk 'NR==FNR {chr[$3]=$1; pos[$3]=$2; next}
NR>FNR {if($3 in chr) {$1=chr[$3]; $2=pos[$3]} print}' \
t1d_lifted_simple.txt t1d_base_data.txt > t1d_base_lifted_final.txt
